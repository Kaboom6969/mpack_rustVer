...
- **Status**: The track stack lives in `src/ffi/stubs/track.rs` (push/pop/element/bytes/str_bytes_all/check_empty/destroy). Reader init/destroy/`done_type`/`read_tag`/`peek_tag`/`read_bytes`/`skip`/`remaining`/discard and Writer init/destroy/write/header/bytes/builder paths are wired. Growable `mpack_writer_init_growable` matches C on track-init failure: keep the buffer, sticky-error, and still wire flush/teardown so destroy reclaims without a dangling pointer.
